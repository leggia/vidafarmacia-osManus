...
async function testSyncBagoFinal() {
  console.log("🔍 Prueba FINAL: Sincronización de factura Bago con tipo FACTURA\n");
  ...
  try {
    ...
    // Registrar compra con tipo FACTURA
    console.log("🔄 Registrando compra en inventarios365.com...\n");
    const result = await inventarios365.registrarCompra({
      proveedor: "Bago",
      tipoComprobante: "FACTURA",  // ← FACTURA en lugar de BOLETA
      numComprobante: "139167",
      almacenNombre: "ALMACEN PRINCIPAL",  // ← Almacén específico
      items,
      total: 2040.30,
    });
